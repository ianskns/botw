	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Olá @${num.split('@')[0]}\nSelamat seja bem vindo leia as regras*${mdata.subject}*`
				...
				teks = `tchau @${num.split('@')[0]} ate mais👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				...
				success: '✔️ pronto ✔️',
				error: {
					stick: '❌ erro ao converter a imagem em um adesivo❌',
					Iv: '❌ Link inválido❌'
				},
				only: {
					group: '❌ Este comando só pode ser usado em grupos! ❌',
					ownerG: '❌ Este comando só pode ser usado pelo proprietário!! ❌',
					ownerB: '❌ Este comando só pode ser usado pelo grupo proprietário!! ❌',
					admin: '❌ Este comando só pode ser usado por administradores de grupo! ❌',
					Badmin: '❌ Este comando só pode ser usado por administradores de grupo! ❌'
				}
			}

			const botNumber = client.user.jid
			const ownerNumber = ["63992896296@s.whatsapp.net"] // replace this with your number
			...
			switch(command) {
				...
				case 'gtts':
					...
					break
				case 'meme':
					meme = await fetchJson('https//kagchi-api.glitch.me/meme/', { method: 'get' })
